...
## 1a. How does total exotic percent cover change with age of pool? (boxplots)
## 1b. How does total exotic diversity change with age of pool? (boxplots)
## 1c. Is there a threshold (# of years) after which exotic abundance increases? (linear model)
## 1d. Do pools converge on the same exotic community composition over time? (NMDS, cluster)
##1e. Is the whole community changing? (NMDS, cluster)
##1f. Is the native community changing? (NMDS, cluster, column graphs)

# 2. What environmental filters can be manipulated to decrease exotics?
## 2a. How does the seedbank compare to the aboveground cover? (presence/absence column graphs, NMDS, cluster)
## 2b. Does active management result in a different exotic community? (NMDS w/ old Del Sol data, cluster)
...
## 3a. Same as 1c -- is there a threshold after which exotics increase (i.e., what's the ideal duration of monitoring?)?
...
## Calculate total exotic % cover per quadrat, find average total exotic % cover per zone per pool per month, add time since restoration column
np_monthly_total_exotic <- np_percent_cover %>%
  filter(Native_Status == "E") %>% 
  group_by(date, Year, month_number, Plot, Replicate, Replicate_Zone) %>% 
  summarize(total_exotic_pc = sum(percent_cover)) %>% 
  drop_na() %>% 
  group_by(date, month_number, Year, Plot, Replicate_Zone) %>% 
  summarize(avg_monthly_exotic_pc = mean(total_exotic_pc)) %>% 
  full_join(creation_year_df) %>% 
  mutate(Creation_Year = as.numeric(as.character(Creation_Year))) %>% 
  mutate(Time_Since = (Year - Creation_Year) + ((month_number-1)/12))

### Check for normality
exotic_pc_hist <- ggplot(np_monthly_total_exotic, aes(x = avg_monthly_exotic_pc)) +
  geom_histogram()
#not normal -- skewed right
exotic_pc_qq <- ggplot(np_monthly_total_exotic, aes(sample = avg_monthly_exotic_pc)) +
  geom_qq()
#not normal -- skewed right

## Scatterplot of total exotic pc per zone, over time
exotic_pc_time_scatter <- np_monthly_total_exotic %>% 
  ggplot(aes(x = Time_Since, y = avg_monthly_exotic_pc)) +
  geom_point() +
  geom_smooth(method = "glm", se = TRUE, size = .5) +
  facet_wrap(~Replicate_Zone) +
  labs(title = "Total Percent Cover of Exotics, over time", x = "Age of Pool (years)", y = "Percent Cover (%)", caption = "Increase in median after year 4 \n driven by sig increase in transition and upland zones (Kruskal-Wallis p << 0.001)") +
  theme_classic() +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
  scale_y_continuous(expand = c(0,0), limits = c(0, 100))
exotic_pc_time_scatter
...
exotic_nmds_fit_coords <- exotic_nmds_fit %>% 
  ...
  filter(pvals <= 0.1) #extract coordinates for species, only keep species with p-val <= 0.1
#POMO p = 0.089, ATSE p = 0.058, SATR p = 0.001, PSUAL p = 0.068, POAR p = 0.002
