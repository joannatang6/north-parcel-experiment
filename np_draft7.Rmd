...
exotic_pc_glmer <- glmer.nb((max_annual_exotic_pc) ~ as.factor(Time_Since) + Replicate_Zone + as.factor(Time_Since):Replicate_Zone + (1|Year) + (1|depth) + (1|size) + (1|plot_replicate), data = np_annual_total_exotic)
#summary(exotic_pc_glmer)
#time, transition, upland sig
#tab_model(exotic_pc_glmer)
#### Check diagnostic graphs
...
#### Continuous GLMER for exotic % cover
exotic_pc_continuous_glmer <- glmer.nb((max_annual_exotic_pc) ~ (Time_Since) + Replicate_Zone + (Time_Since):Replicate_Zone + (1|Year) + (1|depth) + (1|size) + (1|plot_replicate), data = np_annual_total_exotic)
#summary(exotic_pc_continuous_glmer)
#time*transition, time*upland sig
#### Check diagnostic graphs
#E1 <- resid(exotic_pc_continuous_glmer)
#F1 <- fitted(exotic_pc_continuous_glmer)
#plot(x=F1, y=E1, xlab="Fitted Model Values", ylab="Pearson Residuals", main="Variance") #run 2 lines together -- assessing homoskedasticity -- you want to see no pattern
#abline(h=0)
#qqnorm(E1, ylab="Pearson Residuals")
#qqline(E1) #run 2 lines together -- assess if model residuals are normally distributed
#qqnorm(ranef(exotic_pc_continuous_glmer)$plot_replicate[,1], main = "Quadrat Residuals")
#qqline(ranef(exotic_pc_continuous_glmer)$plot_replicate[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(exotic_pc_continuous_glmer)$Year[,1], main = "Monitoring Year Residuals")
#qqline(ranef(exotic_pc_continuous_glmer)$Year[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(exotic_pc_continuous_glmer)$depth[,1], main = "depth Residuals")
#qqline(ranef(exotic_pc_continuous_glmer)$depth[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(exotic_pc_continuous_glmer)$size[,1], main = "size Residuals")
#qqline(ranef(exotic_pc_continuous_glmer)$size[,1]) #run 2 lines together -- assess if random effects residuals are normal
#Checking for overdispersion -- how well your chosen link distribution matches your data
#E1<-resid(exotic_pc_continuous_glmer, type = "pearson")
#N <- length(E1)
#p <- length(coef(exotic_pc_continuous_glmer))
#D <- sum(E1^2) / (N - p)
#D = 0.783 -- want this to be 1.0-1.1; above 1.2 is not great; above 1.3 is really not good (overdispersed = easier to accept false sig differences); if underdispersed (<1) then there's less error than the distribution can handle (harder to detect sig diff)
...
exotic_biomass_glmer <- full_join(annual_exotic_biomass, hydro) %>%
  glmer((annual_biomass+1) ~ as.factor(Time_Since) + Replicate_Zone + as.factor(Time_Since):Replicate_Zone + (1|Year) + (1|depth) + (1|size) + (1|plot_replicate), family = Gamma(link = "log"), data = .)
#summary(exotic_biomass_glmer)
#time, transition, upland sig
#tab_model(exotic_biomass_glmer)
#### Check diagnostic graphs
...
#plot(exotic_pc_lmer_tukey)
#### Continuous GLMER for exotic biomass
exotic_biomass_continuous_glmer <- full_join(annual_exotic_biomass, hydro) %>%
  glmer((annual_biomass+1) ~ (Time_Since) + Replicate_Zone + (Time_Since):Replicate_Zone + (1|Year) + (1|depth) + (1|size) + (1|plot_replicate), family = Gamma(link = "log"), data = .)
#summary(exotic_biomass_continuous_glmer)
#time, transition, upland all sig
#### Check diagnostic graphs
#E1 <- resid(exotic_biomass_continuous_glmer)
#F1 <- fitted(exotic_biomass_continuous_glmer)
#plot(x=F1, y=E1, xlab="Fitted Model Values", ylab="Pearson Residuals", main="Variance") #run 2 lines together -- assessing homoskedasticity -- you want to see no pattern
#abline(h=0)
#qqnorm(E1, ylab="Pearson Residuals")
#qqline(E1) #run 2 lines together -- assess if model residuals are normally distributed
#qqnorm(ranef(exotic_biomass_continuous_glmer)$plot_replicate[,1], main = "Quadrat Residuals")
#qqline(ranef(exotic_biomass_continuous_glmer)$plot_replicate[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(exotic_biomass_continuous_glmer)$Year[,1], main = "Monitoring Year Residuals")
#qqline(ranef(exotic_biomass_continuous_glmer)$Year[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(exotic_biomass_continuous_glmer)$depth[,1], main = "depth Residuals")
#qqline(ranef(exotic_biomass_continuous_glmer)$depth[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(exotic_biomass_continuous_glmer)$size[,1], main = "Msize Residuals")
#qqline(ranef(exotic_biomass_continuous_glmer)$size[,1]) #run 2 lines together -- assess if random effects residuals are normal
...
native_pc_anova <- aov(max_annual_native_pc ~ as.factor(Time_Since) + Replicate_Zone + as.factor(Time_Since):Replicate_Zone + (1|size) + (1|depth) + (1|Year) + (1|as.complex(plot_replicate)), data = np_annual_total_native)
...
native_pc_continuous_lmer <- np_annual_total_native %>% 
  lmer((max_annual_native_pc) ~ (Time_Since) + Replicate_Zone + Time_Since*Replicate_Zone + (1|Year) + (1|size) + (1|depth) + (1|plot_replicate), data = .)
#summary(native_pc_continuous_lmer)
#tab_model(native_pc_continuous_lmer)
#time*transition, time*upland sig
#### Check diagnostic graphs
...
### GLMER for native H'
native_h_glmer <- glmer((max_native_h+1) ~ as.factor(Time_Since) + Replicate_Zone + as.factor(Time_Since):Replicate_Zone + (1|Year) + (1|size) + (1|depth) + (1|plot_replicate), family = Gamma(link = "log"), max_annual_native_h)
#summary(native_h_glmer)
#some age, zone, age:zone sig
#### Check diagnostic graphs
#E1 <- resid(native_h_glmer)
#F1 <- fitted(native_h_glmer)
...
#qqnorm(E1, ylab="Pearson Residuals")
#qqline(E1) #run 2 lines together -- assess if model residuals are normally distributed
#qqnorm(ranef(native_h_glmer)$plot_replicate[,1], main = "Pool Residuals")
#qqline(ranef(native_h_glmer)$plot_replicate[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(native_h_glmer)$Year[,1], main = "Monitoring Year Residuals")
#qqline(ranef(native_h_glmer)$Year[,1]) #run 2 lines together -- assess if random effects residuals are normal
#### ANOVA of lmer
native_h_glmer_anova <- anova(native_h_glmer)
#### Tukey's post-hoc of least-squares means
native_h_glmer_lmer_tukey <- emmeans(native_h_glmer, pairwise ~ as.factor(Time_Since) | Replicate_Zone, adjust = "tukey")
#plot(native_h_glmer_lmer_tukey)
...
## Scatterplot for native H', by zone, by time since
native_h_time_scatter <- max_annual_native_h %>% 
  ggplot(aes(x = (Time_Since), y = max_native_h)) +
  geom_point(color = "darkgreen") +
  geom_smooth(method = lm, se = TRUE, color = "black") +
  facet_wrap(~Replicate_Zone) +
  labs(title = "Native Shannon's Index, over time", x = "Age of Pool (years)", y = "Shannon's Index", caption = "Decrease in 3 in central and upland, \n increase around 4, 7, 9 in central and upland \n increase 4, 9 in transition \n (Sig increase in central and upland over time)") +
  theme_classic() +
  scale_y_continuous(expand = c(0,0), trans = scales::log_trans(), breaks = c(.1, .10, .20, .30, .40, .50, .60, .70, .80, .90, 1.00, 2, 3))
#native_h_time_scatter

### Continuous GLMER for native H'
native_h_continuous_glmer <- glmer((max_native_h+1) ~ (Time_Since) + Replicate_Zone + (Time_Since):Replicate_Zone + (1|Year) + (1|size) + (1|depth) + (1|plot_replicate), family = Gamma(link = "log"), max_annual_native_h)
#summary(native_h_continuous_glmer)
#zone, time*transition sig
#tab_model(native_h_continuous_glmer)
#### Check diagnostic graphs
#E1 <- resid(native_h_continuous_glmer)
#F1 <- fitted(native_h_continuous_glmer)
#plot(x=F1, y=E1, xlab="Fitted Model Values", ylab="Pearson Residuals", main="Variance for Native H'") #run 2 lines together -- assessing homoskedasticity -- you want to see no pattern
#abline(h=0)
#qqnorm(E1, ylab="Pearson Residuals")
#qqline(E1) #run 2 lines together -- assess if model residuals are normally distributed
#qqnorm(ranef(native_h_continuous_glmer)$plot_replicate[,1], main = "Pool Residuals")
#qqline(ranef(native_h_continuous_glmer)$plot_replicate[,1]) #run 2 lines together -- assess if random effects residuals are normal
#qqnorm(ranef(native_h_continuous_glmer)$Year[,1], main = "Monitoring Year Residuals")
#qqline(ranef(native_h_continuous_glmer)$Year[,1]) #run 2 lines together -- assess if random effects residuals are normal
...
## Average natives and nonnative richness per zone
